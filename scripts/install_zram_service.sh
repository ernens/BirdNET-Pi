#!/usr/bin/env bash
# Calculates zram_size as half of the available RAM (rounded up), ensuring it's not less than 2GB.
zram_size=$((($(free -m | awk '/^Mem:/{print $2}') / 2 < 2048 ? 2048 : ($(free -m | awk '/^Mem:/{print $2}') / 2)))
